...
maybe_check_account(#number{assigned_to=AssignedTo, state = <<"port_in">>}=N) ->
    lager:debug("number ~p is assigned to ~p in state port_in"
                ,[N#number.number, AssignedTo]),
    check_account(N);
maybe_check_account(#number{assigned_to=AssignedTo, state = <<"in_service">>}=N) ->
    lager:debug("number ~p is assigned to ~p in state in_service"
                ,[N#number.number, AssignedTo]),
    check_account(N);
...
maybe_check_account(#number{assigned_to=AssignedTo, state=State}=N) ->
    lager:debug("number ~p assigned to acccount id ~p but in state ~p"
                ,[N#number.number, AssignedTo, State]),
